					...
					for (int col = 0; col < 3; col++)
					{
						...
						if (board[i][j] == '.')
						{
							for (int try_fill = 1; try_fill <= 9; try_fill++)
								if (!(block[row_block][col_block][try_fill - 1] || vertical[j][try_fill - 1] || horizontal[i][try_fill - 1]))
...
private:
		...
		vector<vector<unordered_set<int>>> & ready)
	{
		...
		else
		{
			...
			for (auto try_fill : ready[row][col])
			{
				if (!(block[row / 3][col / 3][try_fill - 1] || vertical[col][try_fill - 1] || horizontal[row][try_fill - 1]))
				{
					...
					if (dfs(total_number - 1, row, col, board, horizontal, vertical, block, ready)) return true;
